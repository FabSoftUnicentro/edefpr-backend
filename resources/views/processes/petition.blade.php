...
@section('content')
    @include('processes._formPetition', [
        'form' => $form
    ])
            ...
            <table class="table table-bordered table-condensed">
                ...
                @foreach ($files as $file)
                    <tr class="text-center">
                        ...
                        <td>
                            <a class="btn btn-xs btn-primary" href="{{ route("processes.petitionDownload", [$process->id, $file->id]) }}" target="_blank">
                                Download
                            </a>
    ...
    <script>
        $('#multiple-file-download').on('click', function () {
            ...
            if (selectedFiles.length > 0) {
                var url = "{{ route('processes.petitionDownload', [$process->id, '_files']) }}".replace('_files', selectedFiles.join(','));
        ...
        $('.file-destroy').on('click', function () {
            ...
            .then((value) => {
                switch (value) {
                    case "confirm":
                        $.ajax({
                            url: '{{ route('processes.petitionDestroy', [$process->id, '_file']) }}'.replace('_file', fileId),
                            method: 'DELETE',
        ...
        $('#multiple-file-delete').on('click', function () {
            ...
            if (selectedFiles.length > 0) {
                ...
                .then((value) => {
                    switch (value) {
                        case "confirm":
                            var url = "{{ route('processes.destroy', '_files') }}".replace('_files', selectedFiles.join(','));
